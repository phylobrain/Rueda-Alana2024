...
ggcorrconfplot <- function(corr,
  ...
  conf <- reshape2::melt(conf, na.rm = TRUE)
  colnames(corr) <- c("Var1", "Var2", "value")
  corr$pvalue <- rep(NA, nrow(corr))
  corr$signif <- rep(NA, nrow(corr))
  corr$conf <- conf$value
  ...
  p <-
    ggplot2::ggplot(
      data = corr,
      mapping = ggplot2::aes_string(y = "Var1", x = "Var2", fill = "value")
    )+ scale_x_discrete(position = "top") + scale_y_discrete(limits=rev)+ ggplot2::labs(size = "Confidence\n")
  ...
  if (method == "square") {
    ...
    p <- p +
      ggplot2::geom_point(
        color = outline.color,
        shape = 21,
        ggplot2::aes_string(size = "conf")
      ) + ggplot2::scale_size(range = c(1, 4)) 
