  ...
  ...)
{
  ...
  if (method == 'circle' && plotCI == 'n') {
    symbols(Pos, add = TRUE,  inches = FALSE,
            circles = asp_rescale_factor * 1.6 * abs(PAT) ^ 0.5 / 2,
  ...
  if (method == 'square' && plotCI == 'n') {
    draw_method_square(Pos, PAT, asp_rescale_factor, col.border, col.fill)
  }
