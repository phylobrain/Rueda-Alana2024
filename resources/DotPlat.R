DotPlat<- function (object, assay = NULL, features, cols = c("gold", 
                                                             "dark green"), col.min = -2.5, col.max = 2.5, dot.min = 0, 
                    ...
                    scale.min = NA, scale.max = NA) 
{
  ...
  plot <- ggplot(data = data.plot, mapping = aes_string(x = "features.plot", 
                                                        y = "id")) + geom_point(mapping = aes_string(size = "pct.exp", 
                                                                                                     color = color.by)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + guides(size = guide_legend(title = "Percent Expressed")) + 
  ...
  if (!is.null(x = feature.groups)) {
    plot <- plot + facet_grid(facets = ~feature.groups, scales = "free_x", 
                              space = "free_x", switch = "y") + theme(panel.spacing = unit(x = 0.5, 
